...
const deleteData = (taskId) => {
    axios.delete(`http://localhost:5000/userdelete/${taskId}`);
    window.location.assign('/users');
    alert('User is deleted');
  };
  const deleteorder = (taskId) => {
    axios.delete(`http://localhost:5000/userorderdelete/${taskId}`);
    window.location.assign('/users');
    alert('deleted');
  };
  ...
  const fetchUserBikeData = (userId) => {
    ...
    axios.get(`http://localhost:5000/getorders/${userId}`)
                            ...
                            return (
                              ...
                              >
                                <div style={{ display: 'flex', justifyContent: 'space-around' }}>
                                  <div>
                                    <img src={`http://localhost:5000/${item?.itemImage}`} alt={`${item.itemtype} Image`} style={{ height: "80px" }} />
                                  </div>
                                  <div>
                                    <p>Product Name:</p>
