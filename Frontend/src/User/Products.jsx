...
function Products() {
  ...
  useEffect(() => {
    ...
    if(user){
    axios.get(`http://localhost:5000/wishlist/${user.id}`)
    .then((response) => {
  ...
  const addToWishlist = async (itemId) => {
    try {
      ...
      await axios.post(`http://localhost:5000/wishlist/add`, { itemId: itemId2, title, itemImage,userId,userName });
  ...
  const removeFromWishlist = async (itemId) => {
    try {
      ...
      await axios.post(`http://localhost:5000/wishlist/remove`, { itemId }); // Adjust the endpoint accordingly

      // Refresh the wishlist items
      const user = JSON.parse(localStorage.getItem('user'));
      if(user){
      const response = await axios.get(`http://localhost:5000/wishlist/${user.id}`,); // Adjust the endpoint accordingly
